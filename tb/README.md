...
| **memory.py** | `DictMemory` (byte-addressed dict), `LomeReadOnlyMemory` (Lome adapter: reads from TB memory, logs/no-ops stores), `load_elf()`; constants aligned with `rheon_pkg`. |
...
- **Lome memory integration**: the model is built with `LomeReadOnlyMemory` backed by `DictMemory`. Loads observe the same backing image as RTL-side traffic, while model stores are intentionally non-mutating (intent is still captured in `ChangeRecord.memory_accesses` for scoreboard comparison).
...
### Lome integration notes

- Use **Lome’s README.md** as the primary entrypoint for docs (not only API.md).
- **Error handling**: Architectural exceptions/traps are represented in `ChangeRecord` during `tick()`. Unexpected model failures may still raise; the scoreboard wraps `model.tick(fetch_instr)` in try/except and re-raises with context.
- **PC on exception**: Lome now applies trap PC behavior (e.g. mtvec redirection) during execution. `next_pc` in expected transactions is taken directly from `model.get_pc()` after `tick()`.
- Use canonical `ChangeRecord` fields (`gpr_reads`, `gpr_writes`, `memory_accesses`) when deriving expected transactions.
- Do not rely on Lome’s private state internals (e.g. dict-shaped `_gprs`/`_fprs`); use the public API (`get_pc()`, `get_gpr()`, `tick()`, `poke_pc()`).
...
**ELF from command line:** `make run ELF=path/to.elf`. **CLI commands** (from project shell, use `--test` and `--seed`): `rheon_gen` (generate only); `rheon_sim` (run only, `--test` is elf path); `rheon_run` (generate then run); `rheon_regr` (parallel regressions from `--test NAME,COUNT` or `--file path.yaml`, default `--update 2`, supports `--resume <dir|latest>`, timeout/stop controls, and `--report-json`, see `examples/regression.example.yaml`).

For regression triage, mismatch summaries now include the failing commit `pc`, instruction word, disassembly string, and top mismatched fields.
