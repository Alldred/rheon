...
async def test_elf_loaded(tb: Testbench, log):
    ...
    coverage_archive = Path(
        os.environ.get("RHEON_BUCKET_ARCHIVE", "coverage.bktgz")
    ).expanduser()
    if _env_truthy("RHEON_BUCKET_COVERAGE"):
        from tb.coverage.i_extension import IExtensionCoverage

        coverage_collector = IExtensionCoverage(decoder=tb.model.decoder)
        log.info("Bucket I-extension coverage enabled: %s", coverage_archive)
    ...
    try:
        ...
        if not elf_path:
            log.warning(
                "No ELF found; running from reset vector 0 with short smoke test."
            )
        ...
        while True:
            ...
            if coverage_collector is not None:
                coverage_collector.sample_commit(obj)
        ...
        if coverage_collector is not None:
            exported = coverage_collector.export_archive(coverage_archive)
            log.info("Bucket I-extension coverage archive exported: %s", exported)
